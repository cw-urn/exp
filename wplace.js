...
function applyTheme() {
  ...
  function initializeColorPalette(container) {
    ...
    if (!state.availableColors || state.availableColors.length === 0) {
      ...
      colorsContainer.innerHTML = '<div class="wplace-colors-placeholder">Upload an image first to capture available colors</div>';
      return;
    }
...
async function startRepairMode() {
  ...
  // Ensure we have a paintedMap to check against
  if (!state.paintedMap) {
      Utils.showAlert("No painted progress found to repair. Please paint some of the image first.", "error");
      return;
  }
  ...
  while (state.repairModeActive) {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        // Only check pixels that the bot has marked as painted.
        ...
        // Skip transparent pixels in the source image (redundant with paintedMap check, but safe)
            ...
            if (pixelBatch && pixelBatch.pixels.length > 0) {
              ...
              if (success) {
                pixelBatch.pixels.forEach((p) => { 
                  ...
                  Utils.markPixelPainted(pixel.localX, pixel.localY);
                });
                state.currentCharges -= pixelBatch.pixels.length;
